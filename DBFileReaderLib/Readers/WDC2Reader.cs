            ...
            for (int i = 0; i < fields.Length; i++)
            {
                ...
                if (i == m_reader.IdFieldIndex)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Data.Position = m_columnMeta[i].RecordOffset;
                            Data.Offset = m_dataOffset;
                        }
                ...
                if (!m_reader.Flags.HasFlagExt(DB2Flags.Sparse))
                {
                    Data.Position = m_columnMeta[fieldIndex].RecordOffset;
                    Data.Offset = m_dataOffset;
                }
                ...
                if (info.IsArray)
                {
                    if (arrayReaders.TryGetValue(info.Field.FieldType, out var reader))
                        value = reader(Data, m_recordsOffset, m_fieldMeta[fieldIndex], m_columnMeta[fieldIndex], m_palletData[fieldIndex], m_commonData[fieldIndex], m_reader.StringTable);
                    else
                ...
                else
                {
                    if (simpleReaders.TryGetValue(info.Field.FieldType, out var reader))
                        value = reader(Id, Data, m_recordsOffset, m_fieldMeta[fieldIndex], m_columnMeta[fieldIndex], m_palletData[fieldIndex], m_commonData[fieldIndex], m_reader.StringTable, m_reader);
                    else
